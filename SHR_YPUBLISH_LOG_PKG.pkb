...
IS
    ...
    v_mail_to_email_addr_tab  email_addr_tab_type := email_addr_tab_type( 'nadialee@cisco.com', 'aadvani@cisco.com', 'rruddara@cisco.com', 'aselvara@cisco.com' );
--    v_mail_to_email_addr_tab  email_addr_tab_type := email_addr_tab_type( 'nadialee@cisco.com' );
...
--    v_repost             := get_servlet_repost_flag( in_trans_id, v_rel_id );
    ...
    for j in 1..v_rel_id_tab.count loop
        v_param :=
            v_param ||
            '&releaseNumberId=' || v_rel_id_tab(j);
    end loop;
...
/*
||========================================================================================
|| - For DVDMajorReleaseMDFNavigation ypublish update cspr_cspr_ypublish_dvdmajor_rel_log
||========================================================================================
*/
...
FUNCTION get_rel_number_id( 
            ...
            in_release_number       varchar2)
            
return number  IS

    v_proc_name         varchar2(200) := '[' || g_pkg_name || '.get_rel_number_id] ';
    v_release_number_id            shr_release_number.release_number_id%type;
    
BEGIN
     IF(in_software_type_mdf_id = 280805680
       or  in_software_type_mdf_id = 280805681
       or  in_software_type_mdf_id = 280805682
       or  in_software_type_mdf_id = 280805684
       or  in_software_type_mdf_id = 280805685
     )
    THEN
         select 
               release_number_id   
     ...
     return  v_release_number_id;          
EXCEPTION 
    when others then
        return null;
        
END get_rel_number_id; 
