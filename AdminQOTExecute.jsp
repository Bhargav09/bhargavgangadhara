   ...
   function refreshSearch(){
	...
	function execute(){
   	    ...
   	    if(!releaseNumberCheckedFlag){
   	        alert("At least one release number need to select to submit");
   	        flag = false;
   	    }
   	    ...
   	    for (i=0; i<releaseNumberArray.length; i++){
   	    	...
   	    	if(releaseNumberArray[i].checked){
	   	    	if(migrationPathArray[i].value.length <1 || document.getElementById(migrationDate).value.length<1){
	   	    	     alert("Migration Date and Path must be exist at same time") ;  
	   	    	     flag =false;
	   	    	     break;
	   	        }
	   	    }    
       ...
       for (i=0; i<releaseNumberArray.length; i++){
         ...
         //Ajax check backlog flag, if one image backlog='Y', this release can't be execute.
       if(flag){   
            ...
            for (i=0; i<releaseNumberArray.length; i++){
   	    	...
   	    	if(releaseNumberArray[i].checked){
			 	 ...
			 	 var pars = '&type=BackLogValidation&releaseNumber=' + releaseNumberArray[i].value;
...
<tr>
  ...
  <span class="tabNavData"><span class="tabNavData"><a href="go?action=qotidentify&mode=Identify" id="tabNavLink">Identify</a></span><span class="tabNavMiniDivider"> | </span></span>
    ...
    <span class="tabNavLinkActive"><span class="tabNavData">Execute</span><span class="tabNavMiniDivider"> | </span></span>
...
<input type="hidden" name="mode"  id="mode" value="execute" >
	    ...
	    <tr>
	  	    ...
	  	    <%
	  	 	...
	  	 	<% //List releaseNumberList = (List)request.getAttribute("releaseNumberList"); 
	  	 	   ...
	  	 	   %>
	  	 	<td class="dataTableData" align="center" >
			       	<Select name="releaseNumber" id="releaseNumber"  multiple="multiple" size="6" STYLE="width: 200px" onblur="refreshSearch()">
...
for (int i=0; i<size; i++){ 
   ...
   <tr bgcolor="#ffffff">
       <td align="center" valign="top" rowspan=1><span class="dataTableData">
         ...
         name="selectedReleaseNumberId" id=" id="selectedReleaseNumberId"/>   
 ...
 <!-- @dmarwaha Oct 2012: Add STATUS column value -->
    ...
    %> 
    <td align="center" valign="top" rowspan=1><span class="dataTableData">
          <input type="text" name="migrationPath" size="10"  value='<%=mPath %>'/>  
    </span></td>
         ...
         <table border="1">
            <tr>
           		<td align="left" valign="top" width="%30" NOWRAP><span class="dataTableData">
               			    ...
               			    <INPUT TYPE="RADIO" NAME="imageMigrationPath_<%=j%>" VALUE="<%=imageVo.getMigrationPath()%>" 
               			     checked > <%=imageVo.getMigrationPath()%><BR> 
...
<!-- expand image end  -->
 ...
 <%} %>    
     ...
     <center>
          <img border="0" onclick="javascript: execute()" 
