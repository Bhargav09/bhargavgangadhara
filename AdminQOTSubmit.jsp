...
<%
  ...
  String submitToCCO = "N";
 //String type = (String)request.getParameter("type");
  String type = "";
  //String mode = "Identify";  //default
 ...
 */
	...
	function validateMigrationPathForEachRow(migrationPath,
			releaseNumberAndIndex) {

		var url = 'go?action=qotidentify';
		var pars = '&type=migrationPathValidation&pathToValidate='
				+ migrationPath + '&releaseNumber=' + releaseNumberAndIndex;
	...
	var errorMessage = '';
	function validateMigrationPath(isSubmitToCCO) {

		var submitFlag = true;
		var checkedReleaseNumber = 0;
		var imageFlag = false;
		var submitImage = document.getElementById("btnQotSubmit");

		//only submit one release at a time to CCO
		var releaseNumberArray = document
				.getElementsByName("selectedReleaseNumberId");
		for (i = 0; i < releaseNumberArray.length; i++) {
			if (releaseNumberArray[i].checked) {
				checkedReleaseNumber = checkedReleaseNumber + 1;
				if (checkedReleaseNumber > 1) {
					submitFlag = false;
					alert("only submit one release at a time to CCO");
					// submitImage.src="../gfx/btn_submit.gif";
					break;
				}
		...
		// at least one release number must be checked before submit
		...
		for (i = 0; i < releaseNumberArray.length; i++) {
			...
			if (releaseNumberArray[i].checked) {
				if (migrationPathArray[i].value.length < 1
						|| document.getElementById(migrationDate).value.length < 1) {
		...
		//check expand image case, if image checked but release not check
<%if(request.getParameter("type")!=null && request.getParameter("type").equalsIgnoreCase("expandRelease")){
   	    ...
   	    //check Date is falling on Sunday/Saturday
   	    var releaseNumberArray = document.getElementsByName("selectedReleaseNumberId");
   		for (i=0; i<releaseNumberArray.length; i++){
   	    	
   	    	if(releaseNumberArray[i].checked){
   	    		var migrationDateId = "migrationDate_" +i;
   	    		//var mDate = new Date("Jun 27, 2009");
   	    		var dateString = document.getElementById(migrationDateId).value;
   	    		//alert (dateString);
   	    		var dateArray = dateString.split("-",3);
   	    		var month = dateArray[1];
   	    		var date = dateArray[0];
   	    		var year = dateArray[2];
   	    		var test = month.concat(' ', date, ', ', year);
   	    		var mDate = new Date(test);
   	    		
   	    		//alert(mDate.getDay());
   	    		if(mDate.getDay()==6 || mDate.getDay()==0){
   	    	      	alert(" Migration Date is falling on Sunday/Saturday ");
   	    	      	submitFlag = false;
   	    ...
   	    //validation migration path:  1.release on SDS,   2.Not deferred, 3.Not a migration path for already submitted release
        if(submitFlag){   
            var submitImage = document.getElementById("btnQotSubmit");
            submitImage.src="../gfx/spinner.gif ";
            
            var migrationPathAllArray = new Array(); //image and release
            var j=0;
            var releaseNumber;
            //get release level migration path
            var migrationPathArray = document.getElementsByName("migrationPath");
            for (i=0; i<releaseNumberArray.length; i++){
   	    		if(releaseNumberArray[i].checked){
   	    			migrationPathAllArray[j++] = migrationPathArray[i].value;
   	    			releaseNumber = releaseNumberArray[i].value;
   	    		}
   	    	}
            // alert(migrationPathAllArray.toString());
            //get image level migration path need to do validation 
           <%if(request.getParameter("type")!=null && request.getParameter("type").equalsIgnoreCase("expandRelease")){%>
	      		...
	      		for (var i=0; i<imageArray.length; i++){
	      	    	...
	      	    	if(imageArray[i].checked){  
	      	    	    for (var k=0; k<imageMigrationpath.length; K++){  //radio button array get checked value
	      	    	           if(imageMigrationpath[k].checked)
	      	        	 		migrationPathAllArray[j++] =  imageMigrationpath[k].value;
	      	        	 }
	      	    	}    
	      	 	}
           <%} %>	
  	    	// alert(migrationPathAllArray.toString());
    		 var url = 'go?action=qotidentify';
			 var pars = '&type=migrationPathValidation&pathToValidate=' + migrationPathAllArray.toString() +'&releaseNumber=' + releaseNumber;
    ...
    function showAlert(xmlReq) {  
		 ...
		 if(xmlReq.responseText !=''){
		         ...
		         alert(xmlReq.responseText);
		         var releaseNumberArray = document.getElementsByName("selectedReleaseNumberId");
		         var blankImage = document.getElementsByName("blankImage");
   				 for (i=0; i<releaseNumberArray.length; i++){
   	    	    			if(releaseNumberArray[i].checked)
		         			    blankImage[i].src="../gfx/warning.jpg"
		         }
		 }   	
		 else   {
		 		submitToCCO('Y');
   ...
   function validatePathBeforeSave(isSubmitToCCO){
          
           	validateMigrationPath(isSubmitToCCO);
   }
   ...
   for (i=0; i<releaseNumberArray.length; i++){
   	    	...
   	    	if(releaseNumberArray[i].checked){
   	    		...
   	    		//alert(mDate.getDay());
   	    		if(mDate.getDay()==6 || mDate.getDay()==0){
   	    	      	alert(" Date is falling on Sunday/Saturday ");
   	    		}
   	    	}	
      }
      
   }
   function save(isSubmitToCCO){
   	    var flag = true;
   	    var imageFlag = false;
   	    ...
   	    //release number must checked before save (at least one)   
   	    var releaseNumberArray = document.getElementsByName("selectedReleaseNumberId");
	     ...
	     <%} %>	
   	    ...
   	    if(flag && releaseNumberCheckedFlag){   
   	            if(isSubmitToCCO=='Y')
   	            	document.getElementById("submitToCCO").value="Y";
   				document.getElementById("type").value="save";
   				//alert("Validation is done, submit in progres");
   				document.qotIdentifyForm.submit();
    		}
  	}
  	
  	function submitToCCO(isSubmitToCCO){
  		 if(isSubmitToCCO=='Y')
   	            	document.getElementById("submitToCCO").value="Y";
   				document.getElementById("type").value="save";
   				document.qotIdentifyForm.submit();
...
<tr>
			  ...
			  <span class="tabNavData"><span class="tabNavData"><a href="go?action=qotidentify&mode=Identify" id="tabNavLink">Identify</a></span><span class="tabNavMiniDivider"> | </span></span>
			    <span class="tabNavLinkActive">Submit</span><span class="tabNavMiniDivider"> | </span></span>
...
<input type="hidden" name="submitToCCO"  id="submitToCCO" >
...
<center>
 ...
 <BR>
  <table border="1" width="100%" style="font: 12px Arial; border-collapse: collapse; border: 0.2em solid #BBD1ED ">
      ...
      Select <br/>
        <a href="javascript:checkboxSelectAllHere()">
    ...
    <td align="center" valign="top"><span class="dataTableTitle">
      ...
      <!-- @dmarwaha Oct 2012: Add STATUS column -->
       <td align="center" valign="top"><span class="dataTableTitle">
...
for (int i=0; i<size; i++){ 
   ...
   <tr bgcolor="#ffffff">
       <td align="center" valign="top" rowspan=1><span class="dataTableData">
         ...
         <input type="checkbox" value="<%= vo.getRelease_number_id().toString()%>:<%=i %>" 
         name="selectedReleaseNumberId"  id="selectedReleaseNumberId"/>   
    ...
    <td align="left" valign="top" rowspan=1><span class="dataTableData">
          <img src="../gfx/b1x1.gif"  name="blankImage"/><%= vo.getRelease_number()%></span></td>
    <td align="left" valign="top" rowspan=1><span class="dataTableData">
...
%>
  ...
  <tr bgcolor="#ffffff" align="left">
     <td colspan="22">
      ...
      <table border="1" width="60%" style="font: 12px Arial; border-collapse: collapse; border: 0.2em solid #BBD1ED ">
      <tr bgcolor="#d9d9d9"><span class="dataTableData">
...
<!-- end Sprit 7.5.1  CSCsy32408-->	      
         ...
         <table border="1">
            <tr>
           		<td align="left" valign="top" width="%30" NOWRAP><span class="dataTableData">
         		    
         		    <%
         		        List migrationPathList = imageVo.getMigrationPathList();
         		        for(int t=0; t<migrationPathList.size(); t++){
         		    	   String m_path =(String) migrationPathList.get(t);
         		    	   //System.out.println(" the image migration path is" + m_path);
         		    	   ...
         		    	   //if(migrationPathInDb.equalsIgnoreCase(m_path)) 
         		        	   //isInDb = true;
 ...
 <%} %>    
     ...
     <center>
		<img border="0" name="btnSaveUpdates1" src="../gfx/btn_save_updates.gif" alt="Save Updates" 
			onmouseout="actionBtnSaveImages('btnSaveUpdates1')" onclick="javascript:save('N')" 
			onmouseover="actionBtnSaveImagesOver('btnSaveUpdates1')"/>
		
		<img border="0" onclick="javascript: validatePathBeforeSave('Y')" 
		name="btnQotSubmit" id ="btnQotSubmit" alt="Submit" src="../gfx/btn_submit.gif"/>
	    <!--  a href= "javascript:checkDate()">checkDate</a-->		 
	</center>
 <%} %>
<!--  end of the List -->
